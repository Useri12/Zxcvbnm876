...
local function run(msg,matches)
    ...
    if matches[1] == "وضع صوره للبوت" then
    ...
    if matches[1] == "دخول" then--join by group link
    	local hash = parsed_url(matches[2])
    	import_chat_link(hash,ok_cb,false)
    end
    if matches[1] == "جهات الاتصال" then
    ...
    if matches[1] == "حذف جهة اتصال" then
	    if not is_sudo(msg) then-- Sudo only
    		return
    	end
    ...
    if matches[1] == "اضافة جهة اتصال" and is_sudo(msg) then
    phone = matches[2]
